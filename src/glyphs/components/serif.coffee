				...
				1:
					x:
						...
						then anchors[2].baseRight.x
							...
							contours[0].nodes[2].x - Math.sqrt(Math.abs(1 - serifMedian)) * serifWidth + 20 / (serifCurve + serifHeight + anchors[2].rightWidth * anchors[2].rightCurve) * -(contours[0].nodes[2].x - Utils.onLine({
								y: contours[0].nodes[1].y
								on: [ anchors[2].baseRight, contours[0].nodes[0].point ]
							}))) else Math.min( contours[0].nodes[2].x - serifWidth / 5 - Math.sqrt(Math.abs(1 - serifMedian)) * serifWidth, anchors[0].x + serifHeight + serifCurve + anchors[2].rightWidth * anchors[2].rightCurve )
					y:
						if anchors[2].right == false
				...
				9:
					x:
						...
						then anchors[2].baseLeft.x
							...
							contours[0].nodes[8].x + Math.sqrt(Math.abs(1 - serifMedian)) * serifWidth + 20 / (serifCurve+serifHeight + anchors[2].leftWidth * anchors[2].leftCurve) * -(contours[0].nodes[8].x - Utils.onLine({
								y: contours[0].nodes[9].y
								on: [ anchors[2].baseLeft, contours[0].nodes[10].point ]
							}))) else Math.max( contours[0].nodes[8].x + serifWidth / 5 + Math.sqrt(Math.abs(1 - serifMedian)) * serifWidth, anchors[1].x - serifHeight - serifCurve - anchors[2].leftWidth * anchors[2].leftCurve )
					y:
						if anchors[2].left == false
						then anchors[2].baseLeft.y
						else anchors[2].anchorLine + serifHeight * anchors[2].directionY
